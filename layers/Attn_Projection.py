...
class RotaryProjection(Projection):
    def __init__(self, *, proj_width: int, num_heads: int, max_len: int = 512, base: int = 10000):
        ...
        self.register_buffer("theta", 1.0 / torch.pow(base, torch.arange(0, self.proj_width, 2, dtype=torch.float) / self.proj_width), persistent=False)
...
class QueryKeyProjection(nn.Module):
    def __init__(self, dim: int, num_heads: int, proj_layer, kwargs=None, partial_factor=None):
        ...
        self.key_proj = proj_layer(
            proj_width=self.proj_width,
            num_heads=num_heads,
            **(kwargs or {}),
        )

    @cached_property # 只计算一次，并缓存结果的属性
    ...
    def split_sizes(self):
        ...
        return (int(self.partial_factor[0] * self.head_dim), self.proj_width, int((1.0 - self.partial_factor[1]) * self.head_dim))
